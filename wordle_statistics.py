...
import itertools
import numpy as np
from wordle_main import read_word_files
...
def check_compatibility_word_pattern(guess: str, word: str, pattern: list) -> bool:
    """
    Checks whether a given word 'guess' is compatible with 'pattern' corresponding to 'word' which was given as feedback from Wordle.
...
def vectorized_check_compatibility_word_pattern(guesses: list, word: str, pattern: list) -> list[bool]:
    ...
    # Generate vectorized function for 'word' and 'pattern' which then takes a list of words as input
    func = np.vectorize(lambda x: check_compatibility_word_pattern(x, word, pattern))
    #func = np.vectorize(check_compatibility_word_pattern, otypes = [bool], excluded = [1, 2])

    # Apply func to 'guesses'
    return func(guesses)
...
if __name__ == "__main__":
    ...
    #expected_info = np.array([])
    #for w in solutions:
    #    e = expected_information(w, patterns, allowed_guesses)
    #    expected_info = np.append(expected_info, e)

    exp_inf = np.array([])
    with open("expectation.txt", "r") as f:
        for line in f:
            exp_inf = np.append(exp_inf, float(line))

    print(f"Mean = {np.mean(exp_inf)}. Min = {np.min(exp_inf)} attained for '{solutions[np.argmin(exp_inf)]}'. Max = {np.max(exp_inf)} attained for '{solutions[np.argmax(exp_inf)]}'")
